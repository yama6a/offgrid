...
Proven on `sample-workload-sso.example.com` (protected) vs `sample-workload.example.com` (open).
...
- **Within a domain** (`*.example.com`) → one login, shared via `cookieDomain`.
- **Across domains** (`example.com` → `yama.casa`) → each domain has its own cookie, so the first visit
...
domain** (`sso: "example.com"`), so each domain's policy claims exactly its own routes — no ambiguity
...
- Browse `https://sample-workload-sso.example.com/` (the protected demo from [sample-workload](13_sample_workload.md))
  → Google login → bounce through `google-sso.example.com` → an allowlisted account reaches the sample app; a
  non-listed one is denied.
- `https://sample-workload.example.com/` → the sample app, no login (the open control).
