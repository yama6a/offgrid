...
#   5. 05_argocd.sh                    : bootstrap ArgoCD, which then deploys everything else from git
#   6. 06_restore_sealed_secrets_key.sh: restore the master key so committed SealedSecrets decrypt
#   7. 13_s3_backup_bucket.sh wipe     : DELETE all backups in the bucket, keeping the bucket and IAM
#   8. converge argocd apps            : settle, then drive every app to Synced+Healthy
#   9. 10_ntfy_auth.sh                 : seed ntfy users, seal Grafana's token, push, restart grafana
#  10. verify ingress serving          : wait until each HTTPS host serves an LE cert
#
...
# about-to-be-wiped cluster's key. Back up DELIBERATELY beforehand so step 6 has something to restore. With
# no backup, step 6 fails cleanly and you re-seal instead.
#
# Skips 03a/03b/03c: a reset keeps BOOT/EFI/META, so the OS is already on the NVMe and 03d waits for
# maintenance itself. Steps 1-5 abort on the first failure; 0, 6 and 7 are best-effort.
#
